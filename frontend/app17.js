...
// Mobile dropdown: the query panel collapses to a slim search bar; focusing
// the input (or asking) expands the Q/A log, tapping the map collapses it.
// The collapse styling lives in the mobile media query; .open is a no-op
// on desktop.
const chatPanel = $("chat");
const chatInputEl = $("chat-input");
chatInputEl.addEventListener("focus", () => chatPanel.classList.add("open"));
map.on("click", () => {
...
$("chat-form").addEventListener("submit", async (e) => {
  ...
  const requestSignal = queryAbort.signal;
  clearDataLayer();
  input.value = "";
  hideError();
  chatPanel.classList.add("open");
  log(`You: ${question}`);
  performance.mark("plume-query-start");
  showLoading(true, "Finding the plume…");
  try {
    ...
    log(
      `Plan: ${plan.intent} / ${plan.level} / ${GridRender.prettyVariable(plan.variable)}${cached ? " (cached)" : ""}`
    );
    ...
    if (!requestSignal.aborted) showLoading(false);
  }
});
...
async function renderData(plan, data_url, legend) {
  clearDataLayer();
  if (data_url.startsWith("/frames")) {
    await renderHourly(plan, data_url);
...
let queryAbort = null;
let hourlyAbort = null;
let hourlyLoading = false;
let hourlyCanvas = null;
let hourlyManifest = null;
...
function playAnimation() {
  ...
  animTimer = setInterval(() => {
    if (hourlyLoading && animIndex + 1 >= animFrames.length) return;
...
function paintHourly(frame) {
  const grid = HourlyFrames.grid(hourlyManifest, frame.values, frame.date);
  const buf = GridRender.gridToPixelBuffer(grid, hourlyManifest.range);
  const ctx = hourlyCanvas.getContext("2d");
  ctx.putImageData(new ImageData(buf.data, buf.width, buf.height), 0, 0);
  map.triggerRepaint();
}

async function renderHourly(plan, dataUrl) {
  hourlyAbort = new AbortController();
  const signal = hourlyAbort.signal;
  hourlyLoading = true;
  showLoading(true, "Loading hourly data…");
  async function get(url) {
    for (let attempt = 0; attempt < 3; attempt++) {
      const res = await fetch(`${BACKEND_URL}${url}`, { signal });
      if (res.status !== 429 || attempt === 2) {
        if (!res.ok) throw await httpError(res, "Hourly data unavailable");
        return res;
      }
      const delay = Math.min(60, Number(res.headers.get("Retry-After")) || 2) * 1000;
  ...
  try {
    const manifest = await (await get(dataUrl)).json();
    if (signal.aborted) return;
    hourlyManifest = manifest;
    hourlyCanvas = document.createElement("canvas");
    hourlyCanvas.width = manifest.nx;
    hourlyCanvas.height = manifest.ny;
    animCoords = GridRender.gridCorners(manifest);
    // Sequential six-hour batches bound memory and archive load. Playback
    // begins after the first batch while subsequent batches are downloading.
    for (const batch of manifest.batches) {
      const buffer = await (await get(batch.url)).arrayBuffer();
      if (signal.aborted) return;
      const values = HourlyFrames.decode(buffer, manifest, batch.count);
      const first = animFrames.length === 0;
      values.forEach((v, i) => animFrames.push({ values: v, date: manifest.timestamps[batch.offset + i] }));
      if (first) {
        paintHourly(animFrames[0]);
        savedDataLayer = {
          source: { type: "canvas", canvas: hourlyCanvas, coordinates: animCoords, animate: true },
          layer: { id: "data-layer", type: "raster", source: "data-source",
            paint: { "raster-opacity": .9, "raster-fade-duration": 0 } },
        };
        restoreDataLayer();
        map.fitBounds(GridRender.gridBounds(manifest), { padding: 40 });
        const grid = HourlyFrames.grid(manifest, values[0], manifest.timestamps[0]);
        renderPlumeLegend(grid, GridRender.gridToPixelBuffer(grid, manifest.range));
        $("legend-meta").textContent = `${manifest.time_start} → ${manifest.time_end} · hourly UTC · fixed AOD scale 0–1 (values above 1 saturated)`;
        setupTimeScrubber();
        showFrame(0);
        performance.mark("plume-first-frame");
        performance.measure("plume-first-frame-ms", "plume-query-start", "plume-first-frame");
        playAnimation();
        showLoading(false);
      } else {
        const slider = document.getElementById("time-slider");
        if (slider) slider.max = String(animFrames.length - 1);
      }
    }
    performance.mark("plume-fully-buffered");
    performance.measure("plume-buffered-ms", "plume-query-start", "plume-fully-buffered");
    log(`Loaded ${animFrames.length} hourly frames. ${plan.caption || ""}`);
  } catch (err) {
    if (signal.aborted) return;
    // Retain already-loaded frames; make a partial sequence explicit.
    if (animFrames.length) log(`Only ${animFrames.length} hours loaded; remaining data unavailable.`);
    throw err;
  } finally {
    if (!signal.aborted) { hourlyLoading = false; showLoading(false); }
  }
}
