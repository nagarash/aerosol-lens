 ...
 * instead of blocky), and looped. The timeline scrubs frames; the play
...
function frameUrlFor(data_url, day) {
  ...
  u.searchParams.set("t1", `${day}T00:00:00+00:00`);
...
async function renderPlume(plan, data_url) {
  ...
  try {
    ...
    const range = globalRange(grids);
    animCoords = GridRender.gridCorners(grids[0]);
    let firstBuf = null;
    animFrames = grids.map((grid, i) => {
      const { canvas, buf } = frameCanvas(grid, range);
      if (i === 0) firstBuf = buf;
      return { url: canvas.toDataURL(), date: days[i] };
    });
    addDataLayer(animFrames[0].url, animCoords);
    map.fitBounds(GridRender.gridBounds(grids[0]), { padding: 40 });
    renderPlumeLegend(grids[0], firstBuf);
    setupTimeScrubber();
    playAnimation(); // video plays by default
    log(`Playing ${animFrames.length} daily frames — drag the timeline or pause anytime.`);
  } finally {
    showLoading(false);
  }
}
